...
def interpret_beta_dice(
    ...
    Parameters:
      dice_high: threshold for "high Dice" (task dependent; 0.8 is a common starting point)
      beta_tol: treat |beta| <= beta_tol as "≈ 0" (since exact 0 is rare/noisy)

    Returns: human-readable interpretation string
    """
    if np.isinf(beta):
        if dice > 0:
            # Rare but possible if GT empty and pred overlaps nothing—Dice convention can vary.
...
if __name__ == "__main__":
    ...
    pred3_over = make_ellipsoid(center=(33, 48, 50), radii=(18, 25, 23))
    pred3_under = make_ellipsoid(center=(31, 48, 46), radii=(13, 18, 16))

    spacing_zyx = (2.5, 1.0, 1.0)  # example spacing (z,y,x)
